...
BASE_OUT = DATA_DIR / "tts_input_en_0p25h"
MIN_FREE_MB  = 1500
OVERWRITE    = True
...
def en_builder():
    ...
    audio_dir = BASE_OUT / f"{out_prefix}_{tag}h_wav"
    ds_dir = BASE_OUT / f"{out_prefix}_{tag}h_ds"
    ...
    # If resuming, fast-forward through already “consumed” examples
    ...
    while True:
        ...
        out_wav = audio_dir / f"en_{saved}.wav"
        sf.write(str(out_wav), a, sr, format=AUDIO_FMT, subtype=WAV_SUBTYPE)

        rows.append({"audio": str(out_wav), "text": text})
        total_sec += dur
        saved += 1
